#!/usr/bin/env python3
"""
Staircase Test Logger

Purpose: Capture CSV telemetry from staircase position test
Connects to Arduino Mega 2560 running staircase_test.cpp firmware

Features:
- Auto-detects Arduino serial port or uses specified port
- 500000 baud serial connection
- Creates timestamped CSV file in logs/staircase/ directory
- Mirrors serial output to console
- Robust 5+ minute operation with proper error handling
- Graceful shutdown on Ctrl+C

Usage:
    python3 tools/log_staircase.py                    # Auto-detect port
    python3 tools/log_staircase.py --port /dev/ttyACM0  # Specify port
...
def create_log_directory():
    """
    Create logs/staircase/ directory if it doesn't exist
    Returns the directory path
    """
    # Get script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    log_dir = os.path.join(project_root, 'logs', 'staircase')
...
def create_log_filename(log_dir):
    ...
    Format: staircase_YYYYMMDD-HHMMSS.csv
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
    filename = f'staircase_{timestamp}.csv'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Log staircase test data from Arduino')
    ...
    try:
        with open(log_file, 'w') as f:
            f.write("# Staircase Position Test\n")
            f.write(f"# Port: {port}\n")

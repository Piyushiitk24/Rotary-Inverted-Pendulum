...
def export_tables(report_dir: Path, thesis_tables_dir: Path) -> list[str]:
    ...
    trials_tex = df_to_tabular(
        trials_tbl.round(3),
        caption="Trials overview (thesis identifiers H/N/T).",
...
def export_dataset_table(manifest_path: Path, thesis_tables_dir: Path) -> None:
    """Write a short dataset table mapping thesis IDs to timestamp-coded run IDs."""
    try:
        import json
    ...
    lines.append("\\caption{Dataset used for this thesis (manifest sessions).}")
...
def write_metadata(thesis_dir: Path, manifest: str | None, report_dir: Path, repo_root: Path) -> None:
    ...
    contents = (
        ...
        f"\\renewcommand{{\\ThesisManifest}}{{\\detokenize{{{manifest_str}}}}}\n"
        f"\\renewcommand{{\\ThesisAnalysisReport}}{{\\detokenize{{{report_str}}}}}\n"
        f"\\renewcommand{{\\ThesisAnalysisVersion}}{{\\detokenize{{{git_hash} {ts}}}}}\n"
        "\\renewcommand{\\ThesisFirmwareVersion}{\\detokenize{not recorded}}\n"
    )
    (thesis_dir / "analysis_metadata.tex").write_text(contents, encoding="utf-8")
...
def main() -> int:
    ...
    (thesis_dir / "appendices").mkdir(parents=True, exist_ok=True)
        ...
        try:
            subprocess.check_call(
                [
                    ...
                    "--out",
                    str(thesis_dir / "appendices" / "modelling_derivation_generated.tex"),
                ],
                cwd=repo_root,
            )

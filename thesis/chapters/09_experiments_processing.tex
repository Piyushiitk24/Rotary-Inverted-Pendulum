...
\begin{longtable}{@{}p{0.16\linewidth}p{0.26\linewidth}p{0.12\linewidth}p{0.16\linewidth}p{0.24\linewidth}@{}}
  ...
  Shared & Soft limit margin $\Delta_\theta$ & 2 & \si{\degree} & clamps outward motion for $|\theta|>\SI{80}{\degree}-\Delta_\theta=\SI{78}{\degree}$ \\
  ...
  Full-surface SMC & Internal $\dot{\alpha}$ clamp $\dot{\alpha}_{\mathrm{ctrl},\max}$ & 200 & \si{\degree\per\second} & limits spikes in $\lambda_{\alpha}\dot{\alpha}$ term \\
  ...
  Full-surface SMC & Base term clamp $\theta_{\mathrm{term},\max}$ & 300 & \si{\degree\per\second} & clamps combined base term \\
  Full-surface SMC & Extra accel cap $u_{\max,\mathrm{SMC4}}$ & 12000 & \si{\step\per\second\squared} & tighter shaping than global cap \\
  Full-surface SMC & Extra speed cap $\dot{\theta}_{\max,\mathrm{SMC4}}$ & 2000 & \si{\step\per\second} & tighter shaping than global cap \\
  Full-surface SMC & Base assist gate $|\dot{\alpha}|$ $\dot{\alpha}_{\gamma,\mathrm{SMC4}}$ & 250 & \si{\degree\per\second} & enables assist when still \\
  ...
  Reference tracking & Stability gate $|\alpha|$ $\alpha_{\mathrm{move,win}}$ & 5 & \si{\degree} & required to keep moving profile active \\
  Reference tracking & Stability gate $|\dot{\alpha}|$ $\dot{\alpha}_{\mathrm{move,win}}$ & 80 & \si{\degree\per\second} & required to keep moving profile active \\
  Reference tracking & Engage grace $T_g$ & 200 & ms & before enabling pause/resume gating \\

...
\subsection*{Turning the stepper into a measured axis}
...
\label{sec:bom}
Tables~\ref{tab:bom} and~\ref{tab:bom_passive} list the hardware required to reproduce the rig. The functional design is fixed, but a few procurement details were not finalized when this report was written. Those unresolved entries are kept visible instead of being guessed.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}l c p{0.19\linewidth} p{0.18\linewidth} X@{}}
    ...
    Base encoder 3D-printed housing set & 1 set & Custom CAD print (\texttt{sensor\_housing\_*}) & Converts motor axis into a measured axis & See Figure~\ref{fig:sensor_housing_closed_loop} \\

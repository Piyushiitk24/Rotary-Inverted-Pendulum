...
All controllers are executed in a fixed-period real-time loop on the Arduino Mega 2560. In this project:
\begin{itemize}
  \item the control tick is \SI{200}{Hz} ($T\approx \SI{5}{ms}$),
  \item the high-rate CSV stream is logged at \SI{50}{Hz} (decimated from the control tick).
\end{itemize}

Because timing jitter can affect derivative estimation and controller performance, the firmware monitors:
\begin{itemize}
  \item minimum and maximum observed tick duration,
  \item the count of tick overruns beyond the nominal period.
\end{itemize}

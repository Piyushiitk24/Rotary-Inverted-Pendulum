...
\begin{itemize}
  \item the nominal tick period is set in firmware as \texttt{LOOP\_US}=\SI{5000}{\micro s} (i.e., \SI{200}{Hz}, $T=\SI{5}{ms}$),
  \item each tick measures its actual duration using \texttt{micros()} and uses $dt=\texttt{dtUs}\cdot10^{-6}$ for all discrete-time updates,
...
\begin{itemize}
  ...
  \item the count of tick overruns beyond the nominal period (defined as $dt > \SI{6}{ms}$, i.e., \texttt{dtUs} $>$ \texttt{LOOP\_US}+1000).
\end{itemize}
...
The control law requires $\dot{\alpha}$ and $\dot{\theta}$. Direct finite differences are too noisy at \SI{200}{Hz}, so the firmware uses a first-order filtered differentiator discretized by the bilinear (Tustin) transform:
\[
H(s) = \frac{s}{1+s/\omega_c}.
\]
Applying the bilinear mapping $s \approx \frac{2}{T}\frac{1-z^{-1}}{1+z^{-1}}$ yields the discrete-time recursion
\[
y[n] = b_0\,(x[n]-x[n-1]) - a_1\,y[n-1],
\]
where $x[n]$ is the measured angle (in degrees), $y[n]$ is the filtered derivative (deg/s), and with $T$ the nominal tick period:
\[
b_0 = \frac{2\omega_c}{2+\omega_c T},\qquad
a_1 = \frac{\omega_c T - 2}{2+\omega_c T}.
\]

The cutoff $\omega_c$ (rad/s) is tuned experimentally, and the coefficients $(b_0,a_1)$ are computed using the nominal tick period $T$.

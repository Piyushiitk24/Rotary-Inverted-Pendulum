\section{Hybrid Sliding Mode Controller}
\label{ch:smc}

\subsection{Nonlinear upright dynamics (acceleration input)}
...
\subsection{Sliding surface and motion on the surface}
...
\subsection{Surface dynamics and equivalent control}
...
\subsection{Stepper-oriented implementation constraints}
...
\begin{itemize}
  ...
  \item \textbf{Cosine floor:} replace $\cos\alpha$ by a protected value,
  \[
  \cos_{\mathrm{safe}}
  =\mathrm{sgn}(\cos\alpha)\max(|\cos\alpha|,0.2).
  \]
  \item \textbf{Rate protection:} abort if $|\dot{\alpha}|>\SI{250}{\degree\per\second}$ for the required debounce window.
  \item \textbf{Acceleration cap:} clamp the base acceleration command to
  \[
  u_{\max}=\SI{20000}{\step\per\second\squared}.
  \]
...
\subsection{Base-centering assist term (why this controller is ``hybrid'')}

...
\par
In firmware, the move stability gate is defined as $|\alpha|<\texttt{MOVE\_ALPHA\_WINDOW\_DEG}=\SI{5}{\degree}$ and $|\dot{\alpha}|<\texttt{MOVE\_ALPHADOT\_WINDOW\_DEG\_S}=\SI{80}{\degree\per\second}$, enabled only after a post-engage grace of \texttt{MOVE\_ENGAGE\_GRACE\_MS}=\SI{200}{ms}. Stability/instability is debounced for \texttt{MOVE\_STABLE\_TICKS\_REQ}=\texttt{MOVE\_UNSTABLE\_TICKS\_REQ}=10 consecutive control ticks (\SI{50}{ms} at \SI{200}{Hz}) before pausing/resuming the profile. For safety, user targets are also bounded by $|\theta_{\mathrm{target}}|\le \texttt{THETA\_TARGET\_LIMIT\_DEG}=\SI{35}{\degree}$.
...
\begin{itemize}
  \item \textbf{Deadband on $\alpha$}: \texttt{ALPHA\_DEADBAND\_DEG}=\SI{0.5}{\degree} reduces noise-chasing near upright.
  \item \textbf{Acceleration saturation and engage ramp}: the commanded acceleration is saturated to \texttt{MAX\_ACC\_STEPS}=\SI{20000}{\step\per\second\squared} (and commanded speed to \texttt{MAX\_SPEED\_HZ}=\SI{15000}{\step\per\second}), and multiplied by a linear \SI{100}{ms} engage ramp after \texttt{ENGAGED}.
  \item \textbf{Velocity leak (linear-only)}: \texttt{VEL\_LEAK}=\SI{2.0}{s^{-1}} is enabled only when $|\alpha|<\SI{5}{\degree}$ and $|\theta_{\mathrm{err}}|<\SI{20}{\degree}$ to suppress slow drift (disabled in nonlinear modes).
  \item \textbf{Auto-trim of upright reference}: enabled only when $|\alpha|<\SI{3}{\degree}$ and $|\dot{\alpha}|<\SI{50}{\degree\per\second}$; integrates at \texttt{ALPHA\_TRIM\_RATE}=\SI{0.20}{s^{-1}} and is clamped to $\pm\texttt{ALPHA\_TRIM\_MAX\_DEG}=\SI{1.5}{\degree}$ from the calibration reference.
  \item \textbf{Soft limits}: outward velocity commands are suppressed within \texttt{LIM\_MARGIN\_DEG}=\SI{2}{\degree} of the mechanical base limit.
\end{itemize}

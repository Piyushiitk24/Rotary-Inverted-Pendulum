...
\end{bmatrix}.
\]
For implementation with degree states and stepper units, define the constant conversion $k_s$ (steps per degree; Chapter~\ref{ch:notation}). The commanded acceleration in stepper units is
\[
u_{\mathrm{steps}} = k_s\,u_{\deg}.
\]
Therefore, the gain vector in \emph{steps/s$^2$} per degree (and per deg/s) is
\[
...
\begin{itemize}
  \item \textbf{Deadband on $\alpha$}: a small deadband reduces noise-chasing near upright.
  \item \textbf{Acceleration saturation and engage ramp}: the commanded acceleration is saturated to a safe bound and ramped in during the first \SI{100}{ms} after engage.
  \item \textbf{Velocity leak (linear-only)}: a small leak is applied to the speed integrator near upright to suppress slow drift (disabled in nonlinear modes).
  \item \textbf{Auto-trim of upright reference}: compensates slow bias in $\alpha$ and reduces persistent base offsets.
  \item \textbf{Soft limits}: suppress outward velocity commands near the mechanical bounds.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrl}
    ...
    \midrule
    $K_{\theta}$ & 31.5 & steps/s$^2$ per \si{\degree} & base/arm position gain \\
    $K_{\alpha}$ & 725.6 & steps/s$^2$ per \si{\degree} & pendulum angle gain \\
    $K_{\dot{\theta}}$ & 28.3 & steps/s$^2$ per (\si{\degree\per\second}) & base/arm rate gain \\
    $K_{\dot{\alpha}}$ & 73.5 & steps/s$^2$ per (\si{\degree\per\second}) & pendulum rate gain \\
    \bottomrule
  \end{tabular}
\end{table}
